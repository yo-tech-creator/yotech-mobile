-- ============================================
-- GÖRSEL DENETİM - SCHEDULED HOUR PARAMETRESİ
-- ============================================
-- Görev saati seçimi için p_scheduled_hour parametresi eklendi
...
BEGIN
  ...
  -- Scheduled time hesapla
  v_scheduled_time := (p_scheduled_hour || ':00:00')::TIME;
  
  -- Deadline hesapla
  IF p_scheduled_date = CURRENT_DATE AND p_scheduled_hour <= EXTRACT(HOUR FROM CURRENT_TIME) THEN
    -- Bugün ve seçilen saat geçmişse, şimdiden itibaren hesapla
    v_deadline_at := NOW() + (p_deadline_minutes || ' minutes')::INTERVAL;
  ELSE
    -- Gelecek bir gün/saat için: Seçilen saatten itibaren deadline
    v_deadline_at := (p_scheduled_date::TIMESTAMP + v_scheduled_time) + (p_deadline_minutes || ' minutes')::INTERVAL;
  END IF;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION create_visual_audit_task(UUID[], UUID[], INTEGER, INTEGER, TEXT, DATE, INTEGER) TO authenticated;
