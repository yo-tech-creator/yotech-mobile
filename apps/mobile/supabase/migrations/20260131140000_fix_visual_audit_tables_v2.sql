-- Visual Audit düzeltmeleri v2

-- 1. Eksik tablo: visual_audit_template_branches (many-to-many ilişki tablosu)
CREATE TABLE IF NOT EXISTS visual_audit_template_branches (
...
-- RLS
ALTER TABLE visual_audit_template_branches ENABLE ROW LEVEL SECURITY;

CREATE POLICY "visual_audit_template_branches_select" ON visual_audit_template_branches
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "visual_audit_template_branches_insert" ON visual_audit_template_branches
...
CREATE POLICY "visual_audit_template_branches_delete" ON visual_audit_template_branches
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM visual_audit_templates t 
      WHERE t.id = template_id 
...
-- İndeksler
CREATE INDEX IF NOT EXISTS idx_visual_audit_template_branches_template ON visual_audit_template_branches(template_id);
CREATE INDEX IF NOT EXISTS idx_visual_audit_template_branches_branch ON visual_audit_template_branches(branch_id);

-- 2. visual_audit_templates tablosundaki section_ids ve branch_ids sütunlarını nullable yap
-- (artık ilişkisel tablolar kullanılacak, eski array'ler kaldırılabilir)
ALTER TABLE visual_audit_templates ALTER COLUMN section_ids DROP NOT NULL;

-- 3. create_visual_audit_template fonksiyonunu düzelt (section_ids sütununa değer yazmasın)
...
BEGIN
  ...
  -- Template oluştur (section_ids sütununa değer yazmıyoruz, ilişkisel tablo kullanacağız)
...
-- 4. get_visual_audit_templates_for_manager fonksiyonunu ilişkisel tablolardan okuyacak şekilde güncelle
CREATE OR REPLACE FUNCTION get_visual_audit_templates_for_manager()
RETURNS TABLE (
  id UUID,
  name TEXT,
  description TEXT,
