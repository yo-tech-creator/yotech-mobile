...
COMMENT ON TYPE "public"."store_scoring_item_result" IS 'Form scoring item results: positive (Evet), negative (Hayır), neutral (Kısmen), not_applicable (N/A)';
...
CREATE OR REPLACE FUNCTION "public"."create_visual_audit_task_with_plan"("p_section_ids" "uuid"[], "p_branch_ids" "uuid"[] DEFAULT NULL::"uuid"[], "p_scheduled_hour" integer DEFAULT 9, "p_deadline_minutes" integer DEFAULT 60, "p_min_photos" integer DEFAULT 1, "p_notes" "text" DEFAULT NULL::"text", "p_recurrence" "text" DEFAULT 'daily'::"text", "p_days_of_week" integer[] DEFAULT NULL::integer[], "p_day_of_month" integer DEFAULT NULL::integer) RETURNS json
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_tenant_id UUID;
...
ALTER FUNCTION "public"."create_visual_audit_task_with_plan"("p_section_ids" "uuid"[], "p_branch_ids" "uuid"[], "p_scheduled_hour" integer, "p_deadline_minutes" integer, "p_min_photos" integer, "p_notes" "text", "p_recurrence" "text", "p_days_of_week" integer[], "p_day_of_month" integer) OWNER TO "postgres";
  ...
  LOOP
    ...
    IF v_branches IS NULL OR array_length(v_branches, 1) IS NULL THEN
      ...
      IF v_template.branch_ids IS NULL OR array_length(v_template.branch_ids, 1) IS NULL THEN
        -- Tüm aktif şubeleri al (active sütunu kullan, is_active değil)
        SELECT ARRAY_AGG(b.id) INTO v_branches 
        FROM branches b 
...
COMMENT ON FUNCTION "public"."notify_announcement_published"() IS 'Duyuru veya anket yayınlandığında hedef kitleye push bildirim gönderir';
...
COMMENT ON FUNCTION "public"."send_notification"("p_tenant_id" "uuid", "p_user_ids" "uuid"[], "p_type" "public"."notification_type", "p_title" "text", "p_body" "text", "p_data" "jsonb") IS 'Belirtilen kullanıcılara bildirim gönderir';
...
COMMENT ON FUNCTION "public"."send_notification_to_branch"("p_tenant_id" "uuid", "p_branch_id" "uuid", "p_type" "public"."notification_type", "p_title" "text", "p_body" "text", "p_data" "jsonb", "p_roles" "text"[]) IS 'Bir şubedeki belirtilen rollere bildirim gönderir';
...
COMMENT ON FUNCTION "public"."upload_visual_audit_photo"("p_task_id" "uuid", "p_photo_url" "text", "p_caption" "text", "p_thumbnail_url" "text", "p_latitude" double precision, "p_longitude" double precision) IS 'Visual audit görevi için fotoğraf yükler. template_id NULL olabilir.';
...
COMMENT ON TABLE "public"."notification_queue" IS 'FCM bildirim kuyruğu - Edge Function tarafından işlenir';
...
COMMENT ON TABLE "public"."notifications" IS 'Bildirim geçmişi - kullanıcı bazlı bildirimler';
...
COMMENT ON TABLE "public"."visual_audit_comments" IS 'Görev yorumları ve iletişim';
...
COMMENT ON TABLE "public"."visual_audit_photos" IS 'Yüklenen denetim fotoğrafları';
...
COMMENT ON TABLE "public"."visual_audit_sections" IS 'Görsel denetim bölümleri (Manav, Şarküteri vb.)';
...
COMMENT ON TABLE "public"."visual_audit_tasks" IS 'Günlük oluşan görsel denetim görevleri';
...
COMMENT ON TABLE "public"."visual_audit_templates" IS 'Görsel denetim şablonları - hangi bölümler, hangi saatler';
...
GRANT ALL ON FUNCTION "public"."create_visual_audit_task_with_plan"("p_section_ids" "uuid"[], "p_branch_ids" "uuid"[], "p_scheduled_hour" integer, "p_deadline_minutes" integer, "p_min_photos" integer, "p_notes" "text", "p_recurrence" "text", "p_days_of_week" integer[], "p_day_of_month" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."create_visual_audit_task_with_plan"("p_section_ids" "uuid"[], "p_branch_ids" "uuid"[], "p_scheduled_hour" integer, "p_deadline_minutes" integer, "p_min_photos" integer, "p_notes" "text", "p_recurrence" "text", "p_days_of_week" integer[], "p_day_of_month" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_visual_audit_task_with_plan"("p_section_ids" "uuid"[], "p_branch_ids" "uuid"[], "p_scheduled_hour" integer, "p_deadline_minutes" integer, "p_min_photos" integer, "p_notes" "text", "p_recurrence" "text", "p_days_of_week" integer[], "p_day_of_month" integer) TO "service_role";
